...
data['mid_price'] = (data['best_bid_price'].astype(float) + data['best_ask_price'].astype(float))/2
...
for i_sim in range(n_sim):
    ...
    sigma = 2 #volatility in market
    # Total time.
    T = 1.0
    # Number of steps.
    N = len(data['mid_price'])-1 # int(T/dt)
    # Time step size
    dt = T/N # 0.005
    # Create an empty array to store the realizations.
    s = data['mid_price'].values
    # Initial value of x.
    s[0] = 100
