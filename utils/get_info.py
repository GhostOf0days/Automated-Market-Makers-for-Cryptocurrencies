...
def get_pnl(updates_list: List[Union[MdUpdate, OwnTrade]], ticker_name: str, post_only=False, maker_fee=0, taker_fee=0) -> pd.DataFrame:
    '''
        This function calculates PnL from list of updates
    '''

    # current position in asset and usd
    asset_pos, usd_pos = 0.0, 0.0

    N = len(updates_list)
    asset_pos_arr = np.zeros((N,))
    ...
    for i, update in enumerate(updates_list):
        ...
        asset_pos_arr[i] = asset_pos
        usd_pos_arr[i] = usd_pos
        mid_price_arr[i] = mid_price

    worth_arr = asset_pos_arr * mid_price_arr + usd_pos_arr
    receive_ts = [update.receive_ts for update in updates_list]
    exchange_ts = [update.exchange_ts for update in updates_list]

    df = pd.DataFrame({"exchange_ts": exchange_ts, "receive_ts": receive_ts, "total": worth_arr, ticker_name: asset_pos_arr,
